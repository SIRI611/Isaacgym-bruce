...
class bear_controller:
    def __init__(self):
        # motor manager
        self.BEAR_controller = None

        # motor port and baudrate
        self.BEAR_port = 'COM0'
        self.BEAR_baudrate = 8000000
        ...
        self.dQ  = np.zeros(10)  # raw joint velocity array
        self.dQF = np.zeros(10)  # filtered joint velocity array
        self.IQ  = np.zeros(10)  # raw motor iq array
        self.Tau = np.zeros(10)  # raw joint torque array
        self.dQM = np.zeros(10)  # filtered motor velocity array
        self.trial_max = 2       # BEAR communication maximum trial number for each loop
        ...
        for bear_id in BEAR_LIST:
            ...
            while not check:
                ...
                if bool(ping_rtn[0]):
                    ...
                    if ping_rtn[0][1] != 128:
                        # There is an error:
                        print("ERROR: BEAR %02d in error code: %d!!!" % (bear_id, ping_rtn[0][1]))
                else:
                    trail += 1
                    if trail > 6:
    ...
    def get_error(self):
        ...
        :return bool: Ture if there is any error
        """
        error = False
    ...
    def read_write_force(self, torques, positions, velocities):
        """
        Get present position, velocity and send goal torque, position, velocity to the actuators
        """
    ...
    while True:
        ...
        if loop_start_time - last_damping_check_time > damping_check_duration:
            ...
            if bc.is_damping_mode():
                print(colored('BEAR In Damping Mode! Terminate Now!', 'red'))
                Bruce.stop_threading()
...
if __name__ == "__main__":
    # BEAR Setup
    bc = bear_controller()
    bc.BEAR_port = BEAR_port
    bc.BEAR_baudrate = BEAR_baudrate
    ...
    try:
        ...
        bc.set_damping_mode()
        time.sleep(0.1)
        MM.THREAD_STATE.set({'bear': np.array([2.0])}, opt='only')  # thread is stopped
